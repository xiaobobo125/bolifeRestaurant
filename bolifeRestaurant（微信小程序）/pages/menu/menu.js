...
Page({
    ...
    wx.request({
      ...
      success: res => {
        ...
        for (var i = 0; i < res.data.gtlist.length; i++) {
          gtList.push(res.data.gtlist[i]);
        }
        this.setData({
          tabs: gtList,
          mydata: res.data.gtlist
        })
        console.log(this.data.mydata[0].goods);
        wx.getSystemInfo({
  ...
  pay: function() {
    ...
    for (var i = 0; i < this.data.mydata.length; i++) {
      for (var j = 0; j < this.data.mydata[i].goods.length; j++) {
        if (this.data.mydata[i].goods[j].gcount > 0) {
          foodlist.push(this.data.mydata[i].goods[j])
        }
      }
    }
    this.setData({
